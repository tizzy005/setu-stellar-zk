...
fn init_noncanonical_pub_signals(env: &Env) -> Bytes {
    let mut bytes = Bytes::from_array(env, &[0u8, 0u8, 0u8, 4u8]);
    // First public signal is exactly the BLS12-381 scalar modulus, so it is
    // non-canonical and must be rejected before any set-membership checks.
...
#[test]
fn test_verify_disclosure_without_vk_returns_false() {
    let env = Env::default();
    ...
    assert_eq!(client.verify_disclosure(&proof, &pub_signals), false);
}

#[test]
fn test_set_disclosure_vk_admin_sets_flag() {
    let env = Env::default();
...
#[test]
fn test_verify_disclosure_rejects_malformed_public_signals() {
    let env = Env::default();
    let (_token_id, contract_id, admin) = setup_test_environment(&env);
    let client = PrivacyPoolsContractClient::new(&env, &contract_id);
    ...
    let proof = init_proof(&env);
    let pub_signals = init_malformed_disclosure_pub_signals(&env);

    assert_eq!(client.verify_disclosure(&proof, &pub_signals), false);
}

#[test]
fn test_verify_disclosure_rejects_truncated_bytes() {
    let env = Env::default();
    let (_token_id, contract_id, admin) = setup_test_environment(&env);
    let client = PrivacyPoolsContractClient::new(&env, &contract_id);
    ...
    let proof = Bytes::from_array(&env, &[0u8; 1]);
    let pub_signals = Bytes::from_array(&env, &[0u8; 1]);
...
#[test]
fn test_withdraw_rejects_noncanonical_public_signal() {
    let env = Env::default();
    let (token_id, contract_id, admin) = setup_test_environment(&env);
    let alice = Address::generate(&env);
    let bob = Address::generate(&env);
    ...
    let result = client.withdraw(
        &bob,
        &init_proof(&env),
        &init_noncanonical_pub_signals(&env),
    );
    assert_eq!(
        result,
...
fn test_verify_disclosure_rejects_tampered_recipient_id() {
    ...
    // Tamper recipientId -> discloseHash (index 2) changes
    let mut tampered_signals = pub_signals_struct.pub_signals.clone();
    let disclose_hash = tampered_signals.get(2).unwrap().to_bytes();
    let mut arr = [0u8; 32];
    for i in 0..32 {
        arr[i] = disclose_hash.get(i as u32).unwrap();
    }
    arr[31] ^= 1; // Flip a bit to simulate different recipientId hash
    tampered_signals.set(
        2,
        Fr::from_u256(U256::from_be_bytes(&env, &Bytes::from_array(&env, &arr))),
    );

    let tampered_bytes = PublicSignals {
        pub_signals: tampered_signals,
    }
    .to_bytes(&env);
...
fn test_verify_disclosure_rejects_tampered_purpose() {
    ...
    // Tamper purpose -> discloseHash (index 2) changes
    let mut tampered_signals = pub_signals_struct.pub_signals.clone();
    let disclose_hash = tampered_signals.get(2).unwrap().to_bytes();
    let mut arr = [0u8; 32];
    for i in 0..32 {
    ...
    arr[30] ^= 1; // Flip a different bit to simulate different purpose hash
    tampered_signals.set(
        2,
        Fr::from_u256(U256::from_be_bytes(&env, &Bytes::from_array(&env, &arr))),
    );
